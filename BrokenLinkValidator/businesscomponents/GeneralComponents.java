...
public class GeneralComponents extends ReusableLibrary {
		...
		try	{
			...
			for (int i = 1; i < rowCount+1; i++) {
				...
				if (!row.getCell(0).getStringCellValue().isEmpty()) {
					...
					report.addTestLogSubSection("URL name - " + row.getCell(0).getStringCellValue() + 
					...
					for(int k=0;k<links.size();k++)
					{
						...
						if(url!=null)	{
							if(url.contains("https:"))	{
								...
								report.updateTestLog(url, "Not correct form URL.", Status.FAIL);
								excelRow.createCell(0).setCellValue(k+1);
							...
							report.updateTestLog(url, "No URL present.", Status.FAIL);
							excelRow.createCell(0).setCellValue(k+1);
							excelRow.createCell(1).setCellValue(url);
							excelRow.createCell(2).setCellValue("Invalid");
							excelRow.createCell(3).setCellValue("Not correct form URL.");
	...
	public void verifyLinkActivehttp(String linkUrl, XSSFRow excelRow) 
	{
		HttpURLConnection httpURLConnect = null;
		System.setProperty("org.apache.commons.logging.simplelog.log.org.apache.http.client.protocol.ResponseProcessCookies", "fatel");
		try 
		{
			...
			if(httpURLConnect.getResponseCode()==200)	{			
				...
				report.updateTestLog(linkUrl, "Response Code : " + httpURLConnect.getResponseCode() + "; Response Message : " + 
						httpURLConnect.getResponseMessage(), Status.FAIL);
			}
			else if((httpURLConnect.getResponseCode()==HttpURLConnection.HTTP_NOT_FOUND) || 
			...
			report.updateTestLog(linkUrl, "Connection Timeout.", Status.FAIL);
			excelRow.createCell(1).setCellValue(linkUrl);
	...
	public void verifyLinkActivehttps(String linkUrl, XSSFRow excelRow) 
	{
		...
		try 
		{
			...
			if(httpsURLConnect.getResponseCode()==200)	{
				...
				System.out.println(linkUrl+" - "+httpsURLConnect.getResponseMessage() + " - "+ HttpURLConnection.HTTP_NOT_FOUND);
				report.updateTestLog(linkUrl, "Response Code : " + httpsURLConnect.getResponseCode() + "; Response Message : " + 
						httpsURLConnect.getResponseMessage(), Status.FAIL);
			}
			else if((httpsURLConnect.getResponseCode()==HttpsURLConnection.HTTP_NOT_FOUND) || 
				...
				System.out.println(linkUrl+" - "+httpsURLConnect.getResponseMessage() + " - "+ httpsURLConnect.getResponseCode());				
			}
	...
	public void checkAllInputLinks()	{
		try	{
			...
			if(siteName.equals("Tiki"))	{
				//fileName = "SiteMapUrl_TIKI.xlsx";
				fileName = dataTable.getData("General_Data", "FileName");
			}
			else if(siteName.equals("Zebco"))	{
				//fileName = "SiteMapUrl_ZEBCO.xlsx";
				fileName = dataTable.getData("General_Data", "FileName");
			}
			else if(siteName.equals("Charbroil"))	{
				//fileName = "SiteMapUrl_CB.xlsx";
				fileName = dataTable.getData("General_Data", "FileName");
			}
			else if(siteName.equals("Charbroil EU"))	{
				//fileName = "SiteMapUrl_CB.xlsx";
				fileName = dataTable.getData("General_Data", "FileName");
			}
			else if(siteName.equals("Badlands"))
				fileName = "SiteMapUrl_BP.xlsx";
			...
			DateFormat dateFormat = new SimpleDateFormat(dateFormatString);
			Calendar calendar = Calendar.getInstance();
			String filename = System.getProperty("user.dir") + "\\Results\\Output_"+driver.getTestParameters().getCurrentTestInstance()+"_"
			...
			System.out.println("--------------------------------------------------------------------");
			report.addTestLogSubSection("Total links validated - " + rowCount);
				...
				if (!row.getCell(1).getStringCellValue().isEmpty()) {
					...
					if(url!=null)	{
						...
						if(driver.getPageSource().contains("Fatal error"))	{
							...
							report.updateTestLog(url, "Page contains Fatal error.", Status.FAIL);
	...
	public void invokeApplication() {
		report.updateTestLog("Invoke Application", "Application invoked.", Status.DONE);
	}
